# order by
# select 문에서 result set을 정렬하기 위해 사용하는 구문
# select문의 가장 마지막에 작성되고, 실행순서도 가장 마지막이다
# select문 해석순서 :  from -> where -> group by -> having -> select -> order by

# order by 작성
# asc(오름차순), desc(내림차순)
# 만약 컬럼에 null이 있다면 오름차순으로 정렬
...
# order by 2 desc;  권장하지 않음 가독성 노답
...
# employee 테이블에서 급여 순위 6~10등을 뽑아보자
select emp_name, salary 
from employee
order by salary desc
limit 5,5;

# employee 테이블에서 가장 최근에 입사한 사우너 5명을 뽑아보자
