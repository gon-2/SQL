...
FROM EMPLOYEE
#WHERE EMP_NO LIKE '_______$1%' ESCAPE '$';
...
# REPLACE
SELECT REPLACE('MULTICampus ', 'M', 'AGG');

#TRIM
SELECT TRIM('        MULTICampus      ');
SELECT TRIM('z' from 'zzzzzzMULTICampuszzzzzzzzzzz');
SELECT TRIM(leading 'z' from 'zzzzzzMULTICampuszzzzzzzzzzz');
SELECT TRIM(trailing 'z' from 'zzzzzzMULTICampuszzzzzzzzzzz');
SELECT TRIM(both 'z' from 'zzzzzzMULTICampuszzzzzzzzzzz');
...
# 2. 숫자 관련 함수
# abs, mod, floo, round, ceil, truncate
# abs
select abs(-100);
...
# 이 회사는 30개월간 근무 할 경우 100만원을 지급합니다.
...
# employee테이블에서 사람들의 근속년수를 구해보자
# 퇴사한사람은 x
select emp_name, timestampdiff(year, hire_date, now()) from employee;
