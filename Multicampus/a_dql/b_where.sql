...
# 자바에서 null은 없다는 의미였지만 db에서는 다른 의미를 가짐
# db에서의 null은 알 수 없는 값(undefined, 정의되지 않은 값 이다) 
# null 과 숫자간 산술연산의 결과는 null로 처리한다. / 비교연산도 null로 처리한다
# null 논리연산은?
# true or null -> true( 트루 혹은 널이기때문에 트루 반환)
# false or null -> null
# dual : 함수나 식을 테스트 할때 사용하는 가상테이블
select  10 + null from dual; # 값은 null이 나온다
select 100 > null from dual; # 값은 null이 나온다
select true or null from dual; # true
select false or null from dual; # null
select true and null from dual; # null
select false and null from dual; # false
