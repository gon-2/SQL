...
# 데이터 무결성 보장을 위해 DBMS은 무ㅕㄱㄹ성 제약조건을 제공해준다.
...
# 외래키 제약조건 확인
# DELETE FROM OUR_DEPT WHERE DEPT_CODE = 'D01' => 삭제불가., 자식테이블이 D01로 식별되는 행을 참조하고 있음
# DELETE FROM OUR_DEPT WHERE DEPT_CODE = 'D02' => 삭제 가능.
# UPDATE OUR_DEPT SET DEPT_CODE = 'D01' WHERE DEPT_CODE = 'D01'; => 자식테이블에서 외래키로 사용하고 있는 컬럼은 수정이 불가능
ROLLBACK;

# 외래키 제약조건 확인 2
# 부모테이블의 컬럼에 존재하지 않는 값을 자식테이블의 외래키 컬럼에 넣을 수 없음
-- INSERT INTO OUR_EMP(EMP_ID, EMP_NAME, AGE, EMP_NOP, DEPT_CODE)
-- VALUES(1, '권용구', 19, '000000-0000000', 'D04');

# NULL 제약조건 : NOT NULL이 지정된 EMP_NAME을 뺴고 INSERT
# 실패
-- INSERT INTO OUR_EMP(EMP_ID, AGE, EMP_NO, DEPT_CODE)
-- VALUES(2, 19, '000000-0000000', 'D01');
...
# 실패
-- INSERT INTO OUR_EMP(EMP_ID, EMP_NAME, AGE, EMP_NO, DEPT_CODE)
-- VALUES(3, '배재현', -100, '000000-0000000', 'D01');
