...
# 3. 제약조건은 현재 데이터들과 충돌하지 않는 제약조건만 지정이 가능
...
# ON DELETE SET NULL : 부모테이블에서 자식테이블이 참조하고 있는 행이 삭제 될 경우, 자식테이블의 외래키를 NULL처리
# ON DELETE RESTRICT : 부모 테이블에서 자식테이블이 참조하고 있는 행을 삭제 할 수 없음(DEFAULT)
 ...
 # ON UPDATE SET NULL : 부모테이블에서 자식테이블이 참조하고 있는 행이 수정 될 경우, 자식테이블의 외래키를 NULL처리
